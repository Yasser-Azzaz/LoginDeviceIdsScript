...
def pull_xml(device_id):
    """Pulls the XML to ensure we have the correct structure locally."""
...
def update_device_id_in_xml(new_id):
    """Updates both JsonDeviceID and __Java_JsonDeviceID__ in the local XML."""
...
def force_stop_app(device_id, package):
    run(["adb", "-s", device_id, "shell", "am", "force-stop", package])

def push_and_lock(device_id):
    """Pushes the XML and applies strict 444 permissions + root ownership."""
    run(["adb", "-s", device_id, "push", TEMP_XML_FILE, f"/sdcard/{XML_FILENAME}"])
    # Chain commands to ensure the lock happens immediately
    cmd = f"cp /sdcard/{XML_FILENAME} {XML_PATH} && chown root:root {XML_PATH} && chmod 444 {XML_PATH}"
    run(["adb", "-s", device_id, "shell", "su", "-c", f"'{cmd}'"])

def launch_app(device_id, package):
    run(["adb", "-s", device_id, "shell", "monkey", "-p", package, "-c", "android.intent.category.LAUNCHER", "1"])
...
def main():
    ...
    while True:
        print("\n" + "─" * 40)
        user_input = input("Enter ID Line Number (or 'exit'): ").strip().lower()
        ...
        if target_id:
            ...
            # 1. Force Close MLBB
            print("   🛑 Force closing MLBB...")
            force_stop_app(selected_device, PACKAGE_NAME)

            # 2. Update and Push ID
            update_device_id_in_xml(target_id)
            print("   📤 Pushing ID and locking file (444)...")
            ...
            launch_app(selected_device, PACKAGE_NAME)
            
            print(f"\n✅ ID {target_id} is now running.")
        else:
